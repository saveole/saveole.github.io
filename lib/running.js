...
function resolveRunningDataDir(dir) {
    if (!dir) return null;
    if (fs.existsSync(path.join(dir, 'activities.json'))) return dir;
    if (fs.existsSync(path.join(dir, 'running-data', 'activities.json'))) return path.join(dir, 'running-data');
...
function loadRunningMap(dir) {
    const runningMap = {};
    const resolvedDir = resolveRunningDataDir(dir);
    if (!resolvedDir) return runningMap;

    const file = path.join(resolvedDir, 'activities.json');
    if (fs.existsSync(file)) {
        try {
            ...
            console.warn('loadRunningMap: failed to parse activities.json:', e.message);
        }
    }
    return runningMap;
}
...
function buildRunningPageData(dir) {
    ...
    const actFile = path.join(resolvedDir, 'activities.json');
    const bodyFile = path.join(resolvedDir, 'body.json');
    if (!fs.existsSync(actFile)) return null;

    let activitiesRaw = [];
    try {
        activitiesRaw = JSON.parse(fs.readFileSync(actFile, 'utf-8'));
    } catch (e) {
        console.warn('buildRunningPageData: failed to parse activities.json:', e.message);
        return null;
    }
    ...
    if (fs.existsSync(bodyFile)) {
        try {
            bodyRaw = JSON.parse(fs.readFileSync(bodyFile, 'utf-8'));
        } catch (e) {
            console.warn('buildRunningPageData: failed to parse body.json:', e.message);
        }
    }
...
module.exports = { resolveRunningDataDir, loadRunningMap, mergeRunningMap, buildRunningPageData };